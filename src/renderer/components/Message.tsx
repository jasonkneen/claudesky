...
import ContextFilesList from '@/components/ContextFilesList';
...
export default function Message({ message, isLoading = false }: MessageProps) {
  if (message.role === 'user') {
    ...
    const hasContextFiles = Boolean(message.contextFiles?.length);
    ...
    return (
      <div className="flex justify-end px-1">
        <article className="relative max-w-[min(34rem,calc(100%-2rem))] rounded-2xl border border-neutral-200/70 bg-white px-3 py-2 text-base leading-relaxed text-neutral-900 shadow-sm shadow-black/5 dark:border-neutral-800/60 dark:bg-neutral-900/70 dark:text-neutral-50 dark:shadow-black/30">
          ...
          {hasContextFiles && message.contextFiles && (
            <ContextFilesList contextFiles={message.contextFiles} />
          )}
        </article>
      </div>
    );

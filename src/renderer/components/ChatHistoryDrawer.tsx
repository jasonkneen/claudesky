...
export default function ChatHistoryDrawer({
  ...
  // Create searchable index from conversations (title + all message content)
  const searchIndex = useMemo(() => {
    return conversations.map((conversation) => {
      try {
        ...
        const searchableText = `${conversation.title} ${allText}`.toLowerCase();
        return { id: conversation.id, searchableText, conversation };
      } catch {
        return {
          id: conversation.id,
          searchableText: conversation.title.toLowerCase(),
          conversation
        };
      }
    });
  ...
  const filteredConversations = useMemo(() => {
    ...
    return searchIndex
      .map((entry) => ({
        ...
        score: fuzzyScore(entry.searchableText, query)
      }))
      .filter(({ score }) => score > 0)
      .sort(({ score: a }, { score: b }) => b - a)
      .map(({ conversation }) => conversation)
      .reverse();
  }, [searchQuery, searchIndex, fuzzyScore, conversations]);
          ...
          <div className="relative flex h-full flex-col gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-black/10 bg-white/50 py-2 pr-3 pl-10 text-sm placeholder-neutral-400 transition-colors duration-200 focus:border-black/20 focus:bg-white/80 focus:outline-none dark:border-white/10 dark:bg-white/5 dark:placeholder-neutral-500 dark:focus:border-white/20 dark:focus:bg-white/10"
              />
            </div>

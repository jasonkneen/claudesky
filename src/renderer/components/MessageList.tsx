    ...
    <div ref={containerRef} className={`relative ${containerClasses}`} style={containerStyle}>
      <div className="mx-auto max-w-3xl space-y-1.5">
        {messages.map((message, index) => {
          // Add extra spacing after assistant messages (before user replies)
          const prevMessage = index > 0 ? messages[index - 1] : null;
          const addExtraSpacing = prevMessage?.role === 'assistant' && message.role === 'user';

          return (
            <div key={message.id} className={addExtraSpacing ? 'mt-4' : ''}>
              <Message message={message} isLoading={isLoading && index === messages.length - 1} />
            </div>
          );
        })}
        {isLoading && (
